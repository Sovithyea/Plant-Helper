    ...
    <v-container fluid>
        <v-card class="mb-3">
            ...
            <template v-else>
                ...
                <v-card-subtitle>List all guides.</v-card-subtitle>
            </template>
        </v-card>
        ...
        <v-data-table
            ...
            <template #[`top`]>
                <v-toolbar flat>
                    <v-btn class="ma-1 mr-5" color="primary" :to="{ name: 'activity.create' }"><v-icon left>mdi-plus</v-icon>{{ $t('New Guide') }}</v-btn>
                    <a href="/api/guideExport" style="text-decoration: none;">

                        <v-btn color="success"><v-icon left>mdi-download</v-icon>Export Guide</v-btn>
            ...
            <template #[`item.detail`]="{ item }" :ref="item.id">
               <td v-html="item.detail" class="text-start"></td>
            </template>

            <template #[`item.stage`]="{ item }" :ref="item.id">
               <td> {{item.stage  == 1 ? 'Nursery' : 'Planting'}}   </td>
            </template>
            ...
            <template #[`item.actions`]="{ item }">
                <v-tooltip top>
                    <template #activator="{ on, attrs }">
                        <v-btn color="primary" v-bind="attrs" icon small :to="{ name: 'activity.show', params: { activity: item.id } }" v-on="on"><v-icon>mdi-eye</v-icon></v-btn>
                    </template>
                    <span>Detail</span>
                </v-tooltip>
                <v-tooltip top>
                    <template #activator="{ on, attrs }">
                        <v-btn color="warning" icon small :to="{ name: 'activity.edit', params: { activity: item.id } }" v-bind="attrs" v-on="on"><v-icon>mdi-pencil-box-outline</v-icon></v-btn>
                    </template>
                    <span>Edit</span>
                </v-tooltip>
                <v-tooltip top>
...
export default {
    ...
    methods: {
        ...
        {
            this.deleting = true
            axiosApiInstance.delete(`activity/${ this.deleteItem }`)
            .then( response => {
                this.fetchData()
        ...
        // {
        //     this.exporting = true
        //     axiosApiInstance.get('guideExport', {
        //         headers: {
        //             responseType: 'blob',
        //         }
        //     })
        ...
        //         fileLink.setAttribute('download', 'guides.xlsx');
