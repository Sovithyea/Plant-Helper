    ...
    <v-container fluid>
        <v-card class="mb-3">
            ...
            <template v-else>
                ...
                <v-card-subtitle>List all diseases.</v-card-subtitle>
            </template>
        </v-card>
        ...
        <v-data-table
            ...
            <template #[`top`]>
                <v-toolbar flat>
                    ...
                    <a href="/api/diseaseExport" style="text-decoration: none;">

                        <v-btn color="success"><v-icon left>mdi-download</v-icon>Export Disease</v-btn>
            ...
            <template #[`item.diagnosis`]="{ item }" :ref="item.id">
               <td v-html="item.diagnosis" class="text-start"></td>
            </template>
            ...
            <template #[`item.actions`]="{ item }">
                <v-tooltip top>
                    <template #activator="{ on, attrs }">
                        <v-btn color="primary" v-bind="attrs" icon small :to="{ name: 'disease.show', params: { disease: item.id } }" v-on="on"><v-icon>mdi-eye</v-icon></v-btn>
                    </template>
                    <span>Detail</span>
                </v-tooltip>
                <v-tooltip top>
                    <template #activator="{ on, attrs }">
                        <v-btn color="warning" icon small :to="{ name: 'disease.edit', params: { disease: item.id } }" v-bind="attrs" v-on="on"><v-icon>mdi-pencil-box-outline</v-icon></v-btn>
                    </template>
                    <span>Edit</span>
                </v-tooltip>
                <v-tooltip top>
...
export default {
    ...
    methods: {
        ...
        {
            this.deleting = true
            axiosApiInstance.delete(`disease/${ this.deleteItem }`)
            .then( response => {
                this.fetchData()
        ...
        // {
        //     this.exporting = true
        //     axiosApiInstance.get('diseaseExport', {
        //         headers: {
        //             responseType: 'blob',
        //         }
        //     })
        ...
        //         fileLink.setAttribute('download', 'diseases.xlsx');
