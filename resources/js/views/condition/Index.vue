    ...
    <v-container fluid>
        <v-card class="mb-3">
            ...
            <template v-else>
                ...
                <v-card-subtitle>List all conditions.</v-card-subtitle>
            </template>
        </v-card>
        ...
        <v-data-table
            ...
            <template #[`top`]>
                <v-toolbar flat>
                    <v-btn class="ma-1" color="primary" :to="{ name: 'condition.create' }"><v-icon left>mdi-plus</v-icon>{{ $t('New Condition') }}</v-btn>
                    <v-spacer></v-spacer>
            ...
            <template #[`item.actions`]="{ item }">
                <v-tooltip top>
                    <template #activator="{ on, attrs }">
                        <v-btn color="primary" v-bind="attrs" icon small :to="{ name: 'condition.show', params: { condition: item.id } }" v-on="on"><v-icon>mdi-eye</v-icon></v-btn>
                    </template>
                    <span>Detail</span>
                </v-tooltip>
                <v-tooltip top>
                    <template #activator="{ on, attrs }">
                        <v-btn color="warning" icon small :to="{ name: 'condition.edit', params: { condition: item.id } }" v-bind="attrs" v-on="on"><v-icon>mdi-pencil-box-outline</v-icon></v-btn>
                    </template>
                    <span>Edit</span>
                </v-tooltip>
                <v-tooltip top>
...
export default {
    ...
    methods: {
        async fetchData()
        {
            axiosApiInstance.get('condition', {
                params: {
        ...
        {
            this.deleting = true
            axiosApiInstance.delete(`condition/${ this.deleteItem }`)
            .then( response => {
                this.fetchData()
